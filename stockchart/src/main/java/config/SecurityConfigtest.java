...
public class SecurityConfigtest {
	 // 사용자 정의 SecurityFilterChain 설정
		 ...
		 * .authorizeRequests() .antMatchers("/public/**").permitAll() // /public 경로는 인증
		 * 없이 접근 가능 .anyRequest().authenticated() // 그 외 모든 요청은 인증 필요 .and()
		 * .formLogin() .loginPage("/login") // 커스텀 로그인 페이지 경로 .permitAll() .and()
		 * .logout() .permitAll(); // 로그아웃은 누구나 접근 가능
		 * 
		 * return http.build(); }
		 * 
		 ...
		 * passwordEncoder) { // 사용자 정의 (username, password, roles) 설정 var user1 =
